...
**คุณสมบัติหลัก**: API **คืนข้อมูลทันทีแบบซิงโครนัส** — ไม่ต้อง polling หรือ webhook และยังประมวลผลคำขอพร้อมกันแบบขนานสำหรับงานดึงข้อมูลขนาดใหญ่ สำหรับบริการแบบโฮสต์ จำนวนคำขอที่รันพร้อมกันได้ต่อ API key ขึ้นอยู่กับแพ็กเกจของคุณ (ดู [Concurrency](#การใช้งาน-concurrency-สูง))
...
#### 403 — ฟีเจอร์ไม่มีในแพ็กเกจของคุณ

เฉพาะบริการแบบโฮสต์เท่านั้น คืนค่าเมื่อคำขอใช้ฟีเจอร์ที่แพ็กเกจของคุณไม่รวมไว้ — ปัจจุบันคือ `proxy: "stealth"` ซึ่งต้องใช้แพ็กเกจ Hobby ขึ้นไป
...
#### 429 — ถึงขีดจำกัด concurrency

เฉพาะบริการแบบโฮสต์เท่านั้น คืนค่าเมื่อ API key มีคำขอที่กำลังทำงานอยู่ครบตามจำนวนที่แพ็กเกจอนุญาตแล้ว คำขอนี้จะไม่ถูกเริ่มและไม่มีการหักเครดิต — ลองใหม่เมื่อคำขอที่กำลังทำงานอยู่รายการใดรายการหนึ่งเสร็จสิ้น
...
- **ส่งคำขอแบบขนาน**ภายในขีดจำกัด concurrency ของแพ็กเกจ — คำขอที่เกินขีดจำกัดจะถูกปฏิเสธด้วย `429` แทนการเข้าคิว
...
API ประมวลผลคำขอพร้อมกันแบบขนาน

- **บริการแบบโฮสต์ (anycrawl.dev):** API key แต่ละรายการรันคำขอพร้อมกันได้จำนวนจำกัดตาม[แพ็กเกจ](https://anycrawl.dev/price)ของคุณ คำขอที่เกินขีดจำกัดจะถูกปฏิเสธทันทีด้วย [`429`](#429--ถึงขีดจำกัด-concurrency) — ลองใหม่เมื่อคำขอที่กำลังทำงานอยู่รายการใดรายการหนึ่งเสร็จสิ้น
- **Self-hosted:** ไม่มีขีดจำกัด concurrency โดยค่าเริ่มต้น

รักษาจำนวนคำขอแบบขนานให้อยู่ภายในขีดจำกัดของแพ็กเกจ:
...
ตอบ: สำหรับบริการแบบโฮสต์ มีขีดจำกัด: API key แต่ละรายการรันคำขอพร้อมกันได้จำนวนจำกัดตาม[แพ็กเกจ](https://anycrawl.dev/price)ของคุณ คำขอที่เกินขีดจำกัดจะได้รับ `429 Concurrency limit reached` ทันที — ลองใหม่เมื่อคำขอที่กำลังทำงานอยู่เสร็จสิ้น ส่วนการติดตั้งแบบ self-hosted ไม่มีขีดจำกัด concurrency โดยค่าเริ่มต้น
