...
**핵심**: API는 **즉시 동기적으로** 데이터를 반환합니다 — 폴링이나 웹훅이 필요 없습니다. 대규모 스크래핑을 위해 동시 요청을 병렬로 처리하며, 호스팅 서비스에서는 API 키당 동시에 실행할 수 있는 요청 수가 요금제에 따라 달라집니다([동시성](#높은-동시성-사용) 참고).
...
#### 403 - 요금제에서 지원하지 않는 기능

호스팅 서비스 전용. 요청이 현재 요금제에 포함되지 않은 기능을 사용할 때 반환됩니다. 현재는 `proxy: "stealth"`가 해당하며, Hobby 이상 요금제가 필요합니다.
...
호스팅 서비스 전용. 해당 API 키로 이미 요금제가 허용하는 수만큼 요청이 실행 중일 때 반환됩니다. 요청은 시작되지 않았으며 크레딧도 차감되지 않습니다 — 진행 중인 요청 중 하나가 끝난 후 다시 시도하세요.
...
- **요금제의 동시 요청 한도 내에서 병렬로 요청** — 한도를 넘는 요청은 대기열에 들어가지 않고 `429`로 거부됩니다
...
API는 동시 요청을 병렬로 처리합니다.

- **호스팅 서비스(anycrawl.dev):** API 키마다 동시에 실행할 수 있는 요청 수가 [요금제](https://anycrawl.dev/price)에 따라 제한됩니다. 한도를 넘는 요청은 즉시 [`429`](#429---동시-요청-한도-도달)로 거부됩니다 — 진행 중인 요청 중 하나가 끝난 후 다시 시도하세요.
- **셀프 호스팅:** 기본적으로 동시성 제한이 없습니다.

병렬 요청 수는 요금제 한도 내로 유지하세요:
...
A: 호스팅 서비스에서는 있습니다. API 키마다 동시에 실행할 수 있는 요청 수가 [요금제](https://anycrawl.dev/price)에 따라 제한됩니다. 한도를 넘는 요청은 즉시 `429 Concurrency limit reached`를 반환합니다 — 진행 중인 요청이 끝난 후 다시 시도하세요. 셀프 호스팅 배포는 기본적으로 동시성 제한이 없습니다.
