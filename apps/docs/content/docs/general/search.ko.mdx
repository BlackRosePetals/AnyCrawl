...
호스팅 서비스 전용. `scrape_options`가 현재 요금제에 포함되지 않은 기능을 요청할 때 반환됩니다. 현재는 `proxy: "stealth"`가 해당하며, Hobby 이상 요금제가 필요합니다.
...
- **동시성:** 호스팅 서비스에서는 API 키가 동시에 실행할 수 있는 요청 수가 [요금제](https://anycrawl.dev/price)에 따라 달라지며, 한도를 넘는 요청은 `429`를 반환합니다. 셀프 호스팅 배포는 기본적으로 제한이 없습니다.
