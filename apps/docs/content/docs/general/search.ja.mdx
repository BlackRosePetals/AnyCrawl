...
ホスト型サービスのみ。`scrape_options` でご利用のプランに含まれない機能を指定した場合に返されます。現在は `proxy: "stealth"` が該当し、Hobby 以上のプランが必要です。
...
- **同時実行：** ホスト型サービスでは、API キーごとに同時に実行できるリクエスト数は[プラン](https://anycrawl.dev/price)によって決まり、上限を超えたリクエストは `429` を返します。セルフホスト環境では、既定で上限はありません。
