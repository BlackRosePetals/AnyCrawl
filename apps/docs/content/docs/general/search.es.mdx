...
Solo en el servicio alojado. Se devuelve cuando `scrape_options` solicita una función que tu plan no incluye; actualmente `proxy: "stealth"`, que requiere el plan Hobby o superior.
...
La API procesa en paralelo las búsquedas concurrentes.
...
- **Concurrencia:** en el servicio alojado, el número de solicitudes que una API key puede ejecutar a la vez depende de tu [plan](https://anycrawl.dev/price); las que lo superan devuelven `429`. Las instalaciones autoalojadas no tienen límite por defecto.
