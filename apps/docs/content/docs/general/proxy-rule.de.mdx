...
> **Gehosteter Dienst:** Die explizite Anforderung von `proxy: "stealth"` erfordert den [Hobby-Tarif oder höher](https://anycrawl.dev/price). Im Free-Tarif wird die Anfrage mit `403 stealth_proxy_not_allowed` abgelehnt – auch wenn `stealth` in einem Template, einem Monitor-Ziel oder einer geplanten Aufgabe gesetzt ist. `auto`, `base` und benutzerdefinierte Proxy-URLs sind in jedem Tarif verfügbar. Selbst gehostete Deployments sind nicht eingeschränkt.
