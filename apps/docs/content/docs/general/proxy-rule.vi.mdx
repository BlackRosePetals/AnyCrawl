...
> **Dịch vụ hosted:** yêu cầu tường minh `proxy: "stealth"` cần [gói Hobby trở lên](https://anycrawl.dev/price). Ở gói Free, request sẽ bị từ chối với `403 stealth_proxy_not_allowed` — kể cả khi `stealth` được đặt trong template, target của monitor hoặc tác vụ theo lịch. `auto`, `base` và URL proxy tùy chỉnh dùng được ở mọi gói. Bản self-hosted không bị hạn chế.
