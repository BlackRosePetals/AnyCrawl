...
Somente no serviço hospedado. Retornado quando `scrape_options` solicita um recurso que o seu plano não inclui — atualmente `proxy: "stealth"`, que exige o plano Hobby ou superior.
...
A API processa buscas concorrentes em paralelo.
...
- **Concorrência:** no serviço hospedado, o número de requisições que uma chave de API pode executar ao mesmo tempo depende do seu [plano](https://anycrawl.dev/price); requisições além dele retornam `429`. Instâncias auto-hospedadas não têm limite por padrão.
