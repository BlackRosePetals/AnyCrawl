...
**Ключевые возможности**: API **отвечает сразу синхронно** — без опроса и вебхуков. Параллельные запросы обрабатываются одновременно, что подходит для массового скрейпинга; в облачном сервисе число одновременно выполняемых запросов на один API-ключ зависит от вашего тарифа (см. [Параллельность](#высокая-параллельность)).
...
#### 403 — функция недоступна на вашем тарифе

Только для облачного сервиса. Возвращается, когда запрос использует функцию, не входящую в ваш тариф, — сейчас это `proxy: "stealth"`, для которого нужен тариф Hobby или выше.
...
#### 429 — достигнут лимит параллельности

Только для облачного сервиса. Возвращается, когда у API-ключа уже выполняется столько запросов, сколько допускает тариф. Ничего не было запущено и кредиты не списаны — повторите запрос, когда завершится один из выполняющихся запросов.
...
- **Отправляйте запросы параллельно** в пределах лимита параллельности вашего тарифа — запросы сверх лимита отклоняются с `429`, а не ставятся в очередь
...
API обрабатывает параллельные запросы одновременно.

- **Облачный сервис (anycrawl.dev):** каждый API-ключ может одновременно выполнять ограниченное число запросов в зависимости от вашего [тарифа](https://anycrawl.dev/price). Запрос сверх лимита сразу отклоняется с [`429`](#429--достигнут-лимит-параллельности) — повторите его, когда завершится один из выполняющихся запросов.
- **Self-hosted:** по умолчанию лимита параллельности нет.

Держите число параллельных запросов в пределах лимита вашего тарифа:
...
В облачном сервисе — да: каждый API-ключ может одновременно выполнять ограниченное число запросов в зависимости от вашего [тарифа](https://anycrawl.dev/price). Запросы сверх лимита сразу получают `429 Concurrency limit reached` — повторите, когда завершится выполняющийся запрос. В self-hosted развёртываниях по умолчанию лимита параллельности нет.
