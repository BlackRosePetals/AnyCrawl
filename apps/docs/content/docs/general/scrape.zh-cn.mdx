...
**核心亮点**：API **立即同步返回数据** — 无需轮询或 Webhook。它还会并行处理并发请求，适用于大规模抓取；在托管服务上，每个 API Key 可同时运行的请求数取决于你的套餐（参见[并发](#高并发使用)）。
...
仅限托管服务。当请求使用了你的套餐不包含的功能时返回 —— 目前为 `proxy: "stealth"`，需要 Hobby 及以上套餐。
...
仅限托管服务。当该 API Key 正在运行的请求数已达到套餐允许的上限时返回。请求不会被执行，也不会扣除积分 —— 请在某个进行中的请求完成后重试。
...
- **并行发送请求**，但不要超出套餐的并发上限 —— 超出上限的请求会直接以 `429` 拒绝，而不会排队等待
...
- **托管服务（anycrawl.dev）：** 每个 API Key 可同时运行的请求数有限，具体取决于你的[套餐](https://anycrawl.dev/price)。超出上限的请求会立即以 [`429`](#429---并发数已达上限) 拒绝 —— 请在某个进行中的请求完成后重试。
- **自托管：** 默认不限制并发。
...
答：在托管服务上，是的：每个 API Key 可同时运行的请求数有限，具体取决于你的[套餐](https://anycrawl.dev/price)。超出上限的请求会立即返回 `429 Concurrency limit reached` —— 请在某个进行中的请求完成后重试。自托管部署默认不限制并发。
