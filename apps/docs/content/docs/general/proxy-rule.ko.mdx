...
> **호스팅 서비스:** `proxy: "stealth"`를 명시적으로 요청하려면 [Hobby 이상 요금제](https://anycrawl.dev/price)가 필요합니다. Free 요금제에서는 요청이 `403 stealth_proxy_not_allowed`로 거부되며, 템플릿·모니터 대상·예약 작업 안에서 `stealth`를 설정한 경우도 마찬가지입니다. `auto`, `base`, 사용자 지정 프록시 URL은 모든 요금제에서 사용할 수 있습니다. 셀프 호스팅 배포에는 제한이 없습니다.
