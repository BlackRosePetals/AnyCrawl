...
Только для облачного сервиса. Возвращается, когда `scrape_options` запрашивает функцию, не входящую в ваш тариф, — сейчас это `proxy: "stealth"`, для которого нужен тариф Hobby или выше.
...
API обрабатывает параллельные поисковые запросы одновременно.
...
- **Параллельность:** в облачном сервисе число запросов, которые API-ключ может выполнять одновременно, зависит от вашего [тарифа](https://anycrawl.dev/price); запросы сверх лимита получают `429`. В self-hosted развёртываниях по умолчанию лимита нет.
