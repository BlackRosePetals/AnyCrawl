...
**Destaques**: a API **responde de imediato, de forma síncrona** — sem polling nem webhooks. Ela também processa requisições concorrentes em paralelo para scraping em escala; no serviço hospedado, o número de requisições simultâneas por chave de API depende do seu plano (veja [Concorrência](#alta-concorrência)).
...
Somente no serviço hospedado. Retornado quando a requisição usa um recurso que o seu plano não inclui — atualmente `proxy: "stealth"`, que exige o plano Hobby ou superior.
...
#### 429 — Limite de concorrência atingido

Somente no serviço hospedado. Retornado quando a chave de API já tem tantas requisições em execução quanto o plano permite. Nada foi iniciado e nenhum crédito foi cobrado — tente novamente assim que uma das suas requisições em andamento terminar.
...
- **Envie requisições em paralelo** dentro do limite de concorrência do seu plano — requisições além dele são rejeitadas com `429`, e não enfileiradas
...
A API processa requisições concorrentes em paralelo.

- **Serviço hospedado (anycrawl.dev):** cada chave de API pode executar um número limitado de requisições ao mesmo tempo, conforme o seu [plano](https://anycrawl.dev/price). Uma requisição além desse limite é rejeitada imediatamente com [`429`](#429--limite-de-concorrência-atingido) — tente novamente assim que uma das suas requisições em andamento terminar.
- **Auto-hospedado:** sem limite de concorrência por padrão.

Mantenha o número de requisições paralelas dentro do limite do seu plano:
...
No serviço hospedado, sim: cada chave de API pode executar um número limitado de requisições ao mesmo tempo, conforme o seu [plano](https://anycrawl.dev/price). Requisições além do limite retornam `429 Concurrency limit reached` imediatamente — tente novamente assim que uma requisição em andamento terminar. Instâncias auto-hospedadas não têm limite de concorrência por padrão.
