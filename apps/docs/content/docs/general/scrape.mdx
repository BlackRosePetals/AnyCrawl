...
**Key Features**: The API **returns data immediately and synchronously** - no polling or webhooks required. It also processes concurrent requests in parallel for large-scale scraping; on the hosted service the number running at once per API key depends on your plan (see [Concurrency](#high-concurrency-usage)).
...
Hosted service only. Returned when a request uses a feature your plan does not include — currently `proxy: "stealth"`, which requires Hobby or above.
...
Hosted service only. Returned when the API key already has as many requests running as its plan allows. Nothing was started and no credits were charged — retry once one of your in-flight requests finishes.
...
- **Send requests in parallel** within your plan's concurrency limit — requests beyond it are rejected with `429` rather than queued
...
The API processes concurrent requests in parallel.

- **Hosted service (anycrawl.dev):** each API key can run a limited number of requests at the same time, depending on your [plan](https://anycrawl.dev/price). A request beyond that limit is rejected immediately with [`429`](#429---concurrency-limit-reached) — retry it once one of your in-flight requests finishes.
- **Self-hosted:** no concurrency limit by default.

Keep the number of parallel requests within your plan's limit:
...
A: On the hosted service, yes: each API key can run a limited number of requests at the same time, depending on your [plan](https://anycrawl.dev/price). Requests beyond the limit return `429 Concurrency limit reached` immediately — retry once an in-flight request finishes. Self-hosted deployments have no concurrency limit by default.
