...
**主な特徴**：API は**すぐに同期的に結果を返します**。ポーリングや Webhook は不要です。同時リクエストは並列に処理されるため大規模スクレイプに適していますが、ホスト型サービスでは API キーごとに同時に実行できるリクエスト数がプランによって決まります（[同時実行](#高同時実行)を参照）。
...
#### 403 — ご利用のプランでは利用できない機能

ホスト型サービスのみ。ご利用のプランに含まれない機能をリクエストで使用した場合に返されます。現在は `proxy: "stealth"` が該当し、Hobby 以上のプランが必要です。
...
ホスト型サービスのみ。その API キーで実行中のリクエスト数がすでにプランの上限に達している場合に返されます。リクエストは開始されず、クレジットも消費されません。実行中のリクエストのいずれかが完了してから再試行してください。
...
- **プランの同時実行上限の範囲内でリクエストを並列送信** — 上限を超えたリクエストはキューに入らず `429` で拒否されます
...
API は同時リクエストを並列に処理します。

- **ホスト型サービス（anycrawl.dev）：** API キーごとに同時に実行できるリクエスト数には、[プラン](https://anycrawl.dev/price)に応じた上限があります。上限を超えたリクエストは [`429`](#429--同時実行数の上限に到達) で即座に拒否されます。実行中のリクエストのいずれかが完了してから再試行してください。
- **セルフホスト：** 既定では同時実行数の上限はありません。

並列リクエスト数はプランの上限内に収めてください：
...
A: ホスト型サービスではあります。API キーごとに同時に実行できるリクエスト数には、[プラン](https://anycrawl.dev/price)に応じた上限があります。上限を超えたリクエストは即座に `429 Concurrency limit reached` を返します。実行中のリクエストが完了してから再試行してください。セルフホスト環境では、既定で同時実行数の上限はありません。
