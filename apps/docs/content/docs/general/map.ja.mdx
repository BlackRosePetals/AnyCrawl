...
A: ホスト型サービスでは、API キーごとに同時に実行できるリクエスト数には、[プラン](https://anycrawl.dev/price)に応じた上限があります。上限を超えたリクエストは即座に `429 Concurrency limit reached` を返します。実行中のリクエストが完了してから再試行してください。セルフホスト環境では、既定で同時実行数の上限はありません。
