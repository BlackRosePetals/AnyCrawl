...
**Características clave**: la API **devuelve los datos de inmediato y de forma síncrona**; no hace falta polling ni webhooks. También procesa en paralelo las solicitudes concurrentes para scraping a gran escala; en el servicio alojado, el número de solicitudes simultáneas por API key depende de tu plan (consulta [Concurrencia](#uso-con-alta-concurrencia)).
...
#### 403 - Función no disponible en tu plan

Solo en el servicio alojado. Se devuelve cuando una solicitud usa una función que tu plan no incluye; actualmente `proxy: "stealth"`, que requiere el plan Hobby o superior.
...
Solo en el servicio alojado. Se devuelve cuando la API key ya tiene en ejecución tantas solicitudes como permite su plan. No se inició nada ni se cobraron créditos: reintenta cuando termine alguna de tus solicitudes en curso.
...
- **Envía solicitudes en paralelo** dentro del límite de concurrencia de tu plan: las que lo superan se rechazan con `429` en lugar de ponerse en cola
...
La API procesa en paralelo las solicitudes concurrentes.

- **Servicio alojado (anycrawl.dev):** cada API key puede ejecutar un número limitado de solicitudes a la vez, según tu [plan](https://anycrawl.dev/price). Una solicitud que supere ese límite se rechaza de inmediato con [`429`](#429---límite-de-concurrencia-alcanzado); reinténtala cuando termine alguna de tus solicitudes en curso.
- **Autoalojado:** sin límite de concurrencia por defecto.

Mantén el número de solicitudes en paralelo dentro del límite de tu plan:
...
R: En el servicio alojado, sí: cada API key puede ejecutar un número limitado de solicitudes a la vez, según tu [plan](https://anycrawl.dev/price). Las solicitudes que superan el límite devuelven `429 Concurrency limit reached` de inmediato; reintenta cuando termine una solicitud en curso. Las instalaciones autoalojadas no tienen límite de concurrencia por defecto.
