...
> **ホスト型サービス：** `proxy: "stealth"` を明示的に指定するには [Hobby 以上のプラン](https://anycrawl.dev/price)が必要です。Free プランでは、テンプレート、モニターのターゲット、スケジュールタスク内で `stealth` を設定した場合も含め、リクエストは `403 stealth_proxy_not_allowed` で拒否されます。`auto`、`base`、カスタムプロキシ URL はすべてのプランで利用できます。セルフホスト環境には制限はありません。
