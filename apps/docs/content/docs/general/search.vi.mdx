...
Chỉ áp dụng cho dịch vụ hosted. Trả về khi `scrape_options` yêu cầu dùng tính năng mà gói của bạn không bao gồm — hiện tại là `proxy: "stealth"`, yêu cầu gói Hobby trở lên.
...
API xử lý các request tìm kiếm đồng thời một cách song song.
...
- **Đồng thời:** với dịch vụ hosted, số request một API key chạy được cùng lúc tùy thuộc vào [gói](https://anycrawl.dev/price) của bạn; request vượt giới hạn trả về `429`. Bản self-hosted mặc định không giới hạn.
