...
A: On the hosted service, each API key can run a limited number of requests at the same time, depending on your [plan](https://anycrawl.dev/price). Requests beyond the limit return `429 Concurrency limit reached` immediately — retry once an in-flight request finishes. Self-hosted deployments have no concurrency limit by default.
