...
**Điểm nổi bật**: API **trả dữ liệu ngay và đồng bộ** — không cần polling hay webhook. API cũng xử lý song song các request đồng thời cho thao tác thu thập quy mô lớn; với dịch vụ hosted, số request chạy cùng lúc trên mỗi API key tùy thuộc vào gói của bạn (xem [Đồng thời](#dùng-đồng-thời-cao)).
...
#### 403 - Tính năng không có trong gói của bạn

Chỉ áp dụng cho dịch vụ hosted. Trả về khi request dùng tính năng mà gói của bạn không bao gồm — hiện tại là `proxy: "stealth"`, yêu cầu gói Hobby trở lên.
...
Chỉ áp dụng cho dịch vụ hosted. Trả về khi API key đã có số request đang chạy bằng mức tối đa gói của bạn cho phép. Request không được khởi chạy và không bị trừ credit — hãy thử lại khi một trong các request đang chạy hoàn tất.
...
- **Gửi request song song** trong giới hạn đồng thời của gói — request vượt giới hạn sẽ bị từ chối với `429` thay vì được xếp hàng
...
API xử lý các request đồng thời một cách song song.

- **Dịch vụ hosted (anycrawl.dev):** mỗi API key chỉ chạy được một số request cùng lúc có giới hạn, tùy theo [gói](https://anycrawl.dev/price) của bạn. Request vượt giới hạn sẽ bị từ chối ngay với [`429`](#429---đã-đạt-giới-hạn-đồng-thời) — hãy thử lại khi một trong các request đang chạy hoàn tất.
- **Self-hosted:** mặc định không giới hạn đồng thời.

Hãy giữ số request song song trong giới hạn của gói:
...
**Đáp**: Với dịch vụ hosted, có: mỗi API key chỉ chạy được một số request cùng lúc có giới hạn, tùy theo [gói](https://anycrawl.dev/price) của bạn. Request vượt giới hạn sẽ nhận ngay `429 Concurrency limit reached` — hãy thử lại khi một request đang chạy hoàn tất. Bản self-hosted mặc định không giới hạn đồng thời.
