...
**Kernpunkt**: Die API **antwortet sofort und synchron** – kein Polling, keine Webhooks. Parallele Anfragen werden gleichzeitig verarbeitet, auch für groß angelegtes Scraping; im gehosteten Dienst hängt die Zahl gleichzeitig laufender Anfragen pro API-Key von Ihrem Tarif ab (siehe [Parallelität](#hohe-parallelität)).
...
Nur gehosteter Dienst. Wird zurückgegeben, wenn eine Anfrage eine Funktion nutzt, die Ihr Tarif nicht enthält – derzeit `proxy: "stealth"`, das den Hobby-Tarif oder höher erfordert.
...
- **Anfragen parallel senden** – innerhalb des Parallelitätslimits Ihres Tarifs; Anfragen darüber hinaus werden mit `429` abgelehnt statt in eine Warteschlange gestellt
...
Die API verarbeitet parallele Anfragen gleichzeitig.
