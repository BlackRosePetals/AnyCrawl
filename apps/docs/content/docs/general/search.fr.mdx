...
Service hébergé uniquement. Renvoyé lorsque `scrape_options` demande une fonctionnalité non incluse dans votre offre — actuellement `proxy: "stealth"`, qui nécessite l’offre Hobby ou supérieure.
...
L’API traite les recherches concurrentes en parallèle.
...
- **Concurrence :** sur le service hébergé, le nombre de requêtes qu’une clé API peut exécuter simultanément dépend de votre [offre](https://anycrawl.dev/price) ; les requêtes au-delà renvoient `429`. Les déploiements auto-hébergés n’ont pas de limite par défaut.
