...
Hosted service only. Returned when `scrape_options` asks for a feature your plan does not include — currently `proxy: "stealth"`, which requires Hobby or above.
...
The API processes concurrent search requests in parallel.
...
- **Concurrency:** on the hosted service, the number of requests an API key can run at once depends on your [plan](https://anycrawl.dev/price); requests beyond it return `429`. Self-hosted deployments have no limit by default.
