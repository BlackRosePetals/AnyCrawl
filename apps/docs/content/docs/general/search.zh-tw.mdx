...
僅限雲端服務。當 `scrape_options` 請求了你的方案未包含的功能時回傳——目前為 `proxy: "stealth"`，需要 Hobby 或更高方案。
...
- **併發：** 在雲端服務上，API 金鑰可同時執行的請求數量取決於你的[方案](https://anycrawl.dev/price)；超出上限的請求會回傳 `429`。自架部署預設無限制。
