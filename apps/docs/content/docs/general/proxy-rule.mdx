...
| Mode | Description |
|------|-------------|
| `auto` | Automatically decide between base and stealth proxy. Starts with base when available; if base is unavailable or retries/failures occur, it upgrades/falls back to stealth. |
...
> **Hosted service:** explicitly requesting `proxy: "stealth"` requires the [Hobby plan or above](https://anycrawl.dev/price). On the Free plan the request is rejected with `403 stealth_proxy_not_allowed` — including when `stealth` is set inside a template, a monitor target or a scheduled task. `auto`, `base` and custom proxy URLs are available on every plan. Self-hosted deployments are not restricted.
...
## Timeout Behavior (Scrape API)

- Request-level `timeout` always takes priority.
- If `proxy` is `stealth` or `auto` and `timeout` is not provided, AnyCrawl uses `ANYCRAWL_STEALTH_TIMEOUT_MS` (default: `120000`).
- For `base` mode or custom proxy URLs without explicit `timeout`, AnyCrawl uses `60000`.
...
| `ANYCRAWL_PROXY_STEALTH_CREDITS` | Extra credits charged when stealth proxy is used | `5`                                                  |
| `ANYCRAWL_STEALTH_TIMEOUT_MS` | Default timeout for `proxy=stealth` and `proxy=auto` when request `timeout` is omitted | `120000` |
