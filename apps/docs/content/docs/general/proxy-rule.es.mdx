...
> **Servicio alojado:** solicitar explícitamente `proxy: "stealth"` requiere el [plan Hobby o superior](https://anycrawl.dev/price). En el plan Free la solicitud se rechaza con `403 stealth_proxy_not_allowed`, también cuando `stealth` se define dentro de una plantilla, un objetivo de monitor o una tarea programada. `auto`, `base` y las URL de proxy personalizadas están disponibles en todos los planes. Las instalaciones autoalojadas no tienen restricciones.
