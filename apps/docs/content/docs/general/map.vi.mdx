...
A: Với dịch vụ hosted, mỗi API key chỉ chạy được một số request cùng lúc có giới hạn, tùy theo [gói](https://anycrawl.dev/price) của bạn. Request vượt giới hạn sẽ nhận ngay `429 Concurrency limit reached` — hãy thử lại khi một request đang chạy hoàn tất. Bản self-hosted mặc định không giới hạn đồng thời.
