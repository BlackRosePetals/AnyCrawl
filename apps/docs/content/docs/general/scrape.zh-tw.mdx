...
**核心亮點**：API **立即同步回傳資料** — 無需輪詢或 Webhook。它還會平行處理併發請求，適用於大規模抓取操作；在雲端服務上，每個 API 金鑰可同時執行的請求數量取決於你的方案（參見[併發](#高併發使用)）。
...
僅限雲端服務。當請求使用了你的方案未包含的功能時回傳——目前為 `proxy: "stealth"`，需要 Hobby 或更高方案。
...
僅限雲端服務。當該 API 金鑰正在執行的請求數已達方案允許的上限時回傳。此時不會啟動任何處理，也不會扣除點數——請等待任一進行中的請求完成後再重試。
...
- **平行發送請求**，但不要超過方案的併發上限——超出上限的請求會直接以 `429` 拒絕，而不會排入佇列
...
- **雲端服務（anycrawl.dev）：** 每個 API 金鑰可同時執行的請求數量有限，取決於你的[方案](https://anycrawl.dev/price)。超出上限的請求會立即以 [`429`](#429---已達併發上限) 拒絕——請等待任一進行中的請求完成後再重試。
- **自架部署：** 預設無併發限制。
...
答：在雲端服務上，是的：每個 API 金鑰可同時執行的請求數量有限，取決於你的[方案](https://anycrawl.dev/price)。超出上限的請求會立即回傳 `429 Concurrency limit reached`——請等待進行中的請求完成後再重試。自架部署預設無併發限制。
