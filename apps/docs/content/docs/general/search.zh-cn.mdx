...
仅限托管服务。当 `scrape_options` 请求了你的套餐不包含的功能时返回 —— 目前为 `proxy: "stealth"`，需要 Hobby 及以上套餐。
...
- **并发：** 在托管服务上，每个 API Key 可同时运行的请求数取决于你的[套餐](https://anycrawl.dev/price)；超出上限的请求会返回 `429`。自托管部署默认不限制。
