...
В облачном сервисе каждый API-ключ может одновременно выполнять ограниченное число запросов в зависимости от вашего [тарифа](https://anycrawl.dev/price). Запросы сверх лимита сразу получают `429 Concurrency limit reached` — повторите, когда завершится выполняющийся запрос. В self-hosted развёртываниях по умолчанию лимита параллельности нет.
