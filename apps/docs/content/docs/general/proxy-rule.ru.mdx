...
> **Облачный сервис:** явный запрос `proxy: "stealth"` требует [тарифа Hobby или выше](https://anycrawl.dev/price). На тарифе Free запрос отклоняется с `403 stealth_proxy_not_allowed` — в том числе когда `stealth` задан внутри шаблона, цели монитора или запланированной задачи. `auto`, `base` и собственные URL прокси доступны на всех тарифах. Self-hosted развёртывания не ограничены.
