...
**Points clés** : l’API **renvoie les données immédiatement et de façon synchrone** — pas de polling ni de webhooks. Elle traite les requêtes concurrentes en parallèle pour le scraping à grande échelle ; sur le service hébergé, le nombre de requêtes simultanées par clé API dépend de votre offre (voir [Concurrence](#forte-concurrence)).
...
#### 403 — Fonctionnalité non disponible dans votre offre

Service hébergé uniquement. Renvoyé lorsqu’une requête utilise une fonctionnalité non incluse dans votre offre — actuellement `proxy: "stealth"`, qui nécessite l’offre Hobby ou supérieure.
...
#### 429 — Limite de concurrence atteinte

Service hébergé uniquement. Renvoyé lorsque la clé API a déjà autant de requêtes en cours que son offre le permet. Rien n’a été lancé et aucun crédit n’a été débité — réessayez dès que l’une de vos requêtes en cours se termine.
...
- **Envoyez vos requêtes en parallèle** dans la limite de concurrence de votre offre — les requêtes au-delà sont rejetées avec `429` au lieu d’être mises en file d’attente
...
L’API traite les requêtes concurrentes en parallèle.

- **Service hébergé (anycrawl.dev) :** chaque clé API ne peut exécuter qu’un nombre limité de requêtes simultanément, selon votre [offre](https://anycrawl.dev/price). Une requête au-delà de cette limite est immédiatement rejetée avec [`429`](#429--limite-de-concurrence-atteinte) — réessayez-la dès que l’une de vos requêtes en cours se termine.
- **Auto-hébergé :** aucune limite de concurrence par défaut.

Maintenez le nombre de requêtes parallèles dans la limite de votre offre :
...
Sur le service hébergé, oui : chaque clé API ne peut exécuter qu’un nombre limité de requêtes simultanément, selon votre [offre](https://anycrawl.dev/price). Les requêtes au-delà de la limite renvoient immédiatement `429 Concurrency limit reached` — réessayez dès qu’une requête en cours se termine. Les déploiements auto-hébergés n’ont pas de limite de concurrence par défaut.
