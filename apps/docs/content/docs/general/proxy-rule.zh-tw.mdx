...
> **雲端服務：** 明確指定 `proxy: "stealth"` 需要 [Hobby 或更高方案](https://anycrawl.dev/price)。在 Free 方案上，請求會被拒絕並回傳 `403 stealth_proxy_not_allowed`——包括在範本、監控目標或排程任務中設定 `stealth` 的情況。`auto`、`base` 與自訂代理 URL 在所有方案中皆可使用。自架部署不受此限制。
